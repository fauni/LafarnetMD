...
@Component({
  selector: 'app-detailcertificadosmp',
  templateUrl: './detailcertificadosmp.component.html',
  styleUrls: ['./detailcertificadosmp.component.scss']
})

export class DetailcertificadosmpComponent implements OnInit {
  ...
  onLoadCertificado(codigo_certificado: String){
    ...
    this.servCertificados.getCertificadoAnalisisMP(codigo_certificado).subscribe(data => {
      ...
      this.certificado.nombre_fabricante = data['body'].nombre_fabricante;
  ...
  export(cc: String){
    this.servCertificados.getCertificadoMPPDF(cc.replace('/', '|')).subscribe(data => {
      console.log(data);
  ...
  onAnularCertificado(){
    this.servCertificados.anularCertificado(this.codigo_certificado.replace('/', '|')).subscribe(data => {
      this.toast.show(data['message'], 300);
      if(window.confirm('Desea utilizar la información de este certificado en uno nuevo?')){
        this.route.navigate(['/sacc/certificados/mp/addcmp/'+ this.codigo_certificado]);
      }else{
        this.route.navigate(['/sacc/certificados/mp/list/']);
      }
    }, (err: HttpErrorResponse) => {
      if (err.error instanceof Error) {
        console.log('Ocurrio un error:', err.error.message);
