...
@Component({
  selector: 'app-addcertificadoscmp',
  templateUrl: './addcertificadoscmp.component.html',
  styleUrls: ['./addcertificadoscmp.component.scss']
})
export class AddcertificadoscmpComponent implements OnInit {
  ...
  //pu laq: Array<CaracteristicasCertificado> = new Array<CaracteristicasCertificado>();
...
//Fabricantes
  ...
  onLoadFabricantes() {
    this.servProveedor.getFabricantes().subscribe(
      ...
      (err: HttpErrorResponse) => {
        this.toast.show('Error, No se pudo cargar los Fabricantes!', 3000, 'red');
        if (err.error instanceof Error) {
          // A client-side or network error occurred. Handle it accordingly.
          console.log('Ocurrio un error:', err.error.message);
        } else {
