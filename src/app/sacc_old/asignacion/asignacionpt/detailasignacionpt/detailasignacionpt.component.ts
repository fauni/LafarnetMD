...
import { MzToastService, MzModalComponent } from '../../../../../../node_modules/ng2-materialize';
...
export class DetailasignacionptComponent implements OnInit {
  ...
  @ViewChild('clasificacionModal') modal: MzModalComponent;
 ...
 // Variables Caracteristicas de Producto Terminado
  ...
  ngOnInit(): void {
    this.route.params.subscribe(params => {
        ...
        // alert(this.clasificacion_producto);
    ...
    this.servAsignacion.getCaracteristicasForProducto(code).subscribe(
      data => {
        ...
        if (data['length'] > 0) {
          ...
          this.show = false;
          if (this.clasificacion_producto == 'SOLIDOS NO ESTÉRILES' || this.clasificacion_producto == 'LIQUIDOS NO ESTÉRILES' || this.clasificacion_producto == 'SEMISOLIDOS') {
            this.closeLoading();
            this.onLoadClasificacion(this.clasificacion_producto);
          } else {
            this.modal.open();
            // alert('Seleccione una clasificación ' + this.clasificacion_producto);
          }
        }
      },
      (err: HttpErrorResponse) => {
...
// Este metodo guarda los elementos cuando no se tiene caracteristicas asignadas al producto
  ...
  guardarAsignacionCaracteristicaProducto() {
    ...
    this.toast.show('Se realizo la asignación Correctamente!', 3000, 'green', () => this.router.navigate(['/sacc/asignacion/pt/listadd', this.codigo_producto]));
  }
